...
df2 = pd.read_excel('END395_ProjectPartIDataset.xlsx',sheet_name="Pallets2")
...
for j in M.J:
    for i in M.I:
        if (2 <= j <= 21 and 2<=i<=16):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
        if (62 <= j <= 83 and 2<=i<=16):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
        if (84 <= j <= 94 and 2<=i<=16):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
        if (22 <= j <= 39 and 17<=i<=33):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
        if (40 <= j <= 61 and 17<=i<=33):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
        if (95 <= j <= 105 and 17<=i<=33):
            M.constraints.add(sum(M.x[i,j,k] for k in M.K) == 0)
